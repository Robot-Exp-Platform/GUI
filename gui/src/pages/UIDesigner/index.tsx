...
import { Button, Header, Icon, Segment, Modal, Form, List, Message } from "semantic-ui-react";
import { Stage, Layer, Rect } from 'react-konva';
import { UIDesignerProvider, useUIDesigner } from "~/components/contexts/UIDesignerContext";
...
const UIDesignerContent: FC = () => {
  ...
  const handleCanvasClick = (e: any) => {
    ...
    // 如果点击的是 Stage 或 Layer 或背景 Rect，则清除当前选择
    if (targetName === 'Stage' || targetName === 'Layer' || 
  ...
  const renderItems = () => {
    ...
    const sortedItems = [...currentUI.items].sort((a, b) => a.zIndex - b.zIndex);
      ...
      <Modal.Content>
        ...
        <Segment>
          ...
          <Form>
            ...
            />
            <Button primary onClick={handleCreateUI}>创建</Button>
          </Form>
        </Segment>
            ...
            <List divided relaxed>
              {project.config.uiFiles.map((file) => (
                <List.Item key={file.id}>
                  ...
                  <List.Content>
                    <List.Header as="a" onClick={() => handleSelectUI(file.path)}>
                      {file.name}
                    </List.Header>
      ...
      <div className="ui-content-area">
        ...
        {isEditing && selectedItem && (
          <div className="ui-editor-container">
            <Segment>
              <Header as="h4">编辑 {selectedItem.type === 'text' ? '文本' : 
                                   selectedItem.type === 'image' ? '图片' :
                                   selectedItem.type === 'rectangle' ? '矩形' :
                                   selectedItem.type === 'circle' ? '圆形' :
                                   selectedItem.type === 'triangle' ? '三角形' : '组件'}</Header>
              {renderEditor()}
            </Segment>
          </div>
        )}
